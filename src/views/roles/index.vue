...
export default {
  ...
  data() {
    return {
      ...
      rules: {   //表单验证
        roleName: [{ required: true, message: '请输入字段', trigger: 'blur' }],
        roleNickname: [{ required: true, message: '请输入名称', trigger: 'blur' }],
        // longitude: [{ validator: longitudeCheck, trigger: 'change' }],
      },
      dialogFormModel: {}, //表单双向绑定
      dialogFormElement: [  //表单元素
        { label: '角色字段', prop: 'roleName', type: 'input' },
        { label: '角色名称', prop: 'roleNickname', type: 'input' },
